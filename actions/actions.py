...
ALLOWED_PIZZA_SIZES = ["small", "medium", "large", "extra-large"]

ALLOWED_PIZZA_TYPES = ["mozzarella", "pepperoni", "fungi", "mushroom", "veglovers", "hawaii", "meatlovers"]
VEGETARIAN_PIZZAS = ["mozzarella", "fungi", "mushroom", "veglovers"]
MEAT_PIZZAS = ["pepperoni", "meatlovers", "hawaii"]
...
class ValidateSimplePizzaForm(FormValidationAction):
    ...
    def validate_pizza_size(
        ...
        if slot_value.lower() not in ALLOWED_PIZZA_SIZES:
            dispatcher.utter_message(text=f"We only accept pizza sizes: s/m/l/xl/small/medium/large")
    ...
    def validate_pizza_type(
        ...
        if slot_value.lower() not in ALLOWED_PIZZA_TYPES:
            dispatcher.utter_message(text=f"We only accept pizza types: mozzarella/pepperoni/fungi/mushroom/veglovers/hawaii/meatlovers")
...
class AskForPizzaTypeAction(Action):
    ...
    def run(
        ...
        if tracker.get_slot("vegetarian"):
            dispatcher.utter_message(
                text="What kind of vegetarian pizza do you want?",
...
class AskForPizzaSizeAction(Action):
    ...
    def run(
        ...
        dispatcher.utter_message(
            text="What size pizza would you like to order?",
            buttons=[
                {"title": p, "payload": p} for p in ALLOWED_PIZZA_SIZES
            ]
        )
        return []
...
class ValidateFancyPizzaForm(FormValidationAction):
    ...
    def validate_vegetarian(
        ...
        if tracker.get_intent_of_latest_message() == "affirm":
            dispatcher.utter_message(
                text="I'll remember you prefer vegetarian pizza."
            )
    ...
    def validate_pizza_type(
        ...
        if slot_value.lower() not in ALLOWED_PIZZA_TYPES:
            dispatcher.utter_message(text="We only accept pizza types: mozzarella/pepperoni/fungi/mushroom/veglovers/hawaii/meatlovers")
            return {"pizza_type": None}
        dispatcher.utter_message(text=f"OK! You want to have a {slot_value} pizza.")
        return {"pizza_type": slot_value}
